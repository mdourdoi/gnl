...
/*   Updated: 2025/11/19 16:53:03 by mdourdoi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_get_result(char *s, int *error)
{
	int		len;
	char	*result;
...
static char	*ft_get_residue(char *s, int *error)
{
	int		len;
	int		start;
	char	*residue;
...
char	*get_next_line(int fd)
{
	...
	line = NULL;
	result = NULL;
	residue = malloc(1024 * sizeof(char *));
	line = ft_line(fd, residue[fd]);
	if (!line)
		return (ft_free_all(NULL, &residue));
	ft_free_all(residue[fd], NULL);
	result = ft_get_result(line, &error);
	residue[fd] = ft_get_residue(line, &error);
	free(line);
	if (error || ft_strlen(result, 0) == 0)
